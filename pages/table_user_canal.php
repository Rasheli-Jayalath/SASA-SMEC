...
<?php  include '../Classes/get_nc_name.php';  ?>
...
<?php include("../includes/left_menu1.php");?> 
  <main class="main-content mt-1 border-radius-lg">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page"> User Canal Table </li>
          </ol>
          <h5 class="font-weight-bolder mb-0"> User Canal Table</h5>
        </nav>
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          <div class="ms-md-auto pe-md-3 d-flex align-items-center">
          <div class="input-group me-4 ">
              <form action="table_user_canal.php " method="get" id="filter" name="filter" class="">
    ...
    <script>
          ...
          xmlhttp.open("GET","../Scripts/table_ajax/ajax_user_canal.php?q="+str,true);
          xmlhttp.send();
    ...
    $start = ($page_no -1)* $result_per_page;
        $search = '';
        // getting the list of table_user_canal
        if ( isset($_GET['search']) ) {
            $search = mysqli_real_escape_string($connection, $_GET['search']);
            $sql2 = "SELECT * FROM wh_002_users_canal WHERE is_deleted = 0  $yr_name ORDER BY uc_id ";
        } else {
            $sql2 = "SELECT * FROM wh_002_users_canal WHERE is_deleted = 0  $yr_name ORDER BY uc_id   LIMIT {$start}, {$result_per_page}";
        }
        ...
        <colgroup>
                ...
                <tr>
                  <th class=\"text-uppercase text-secondary text-xs font-weight-bolder opacity-7\"> Canal </th>
                  <th class=\"text-uppercase text-secondary text-xs font-weight-bolder opacity-7\"> Main component </th>
                  <th class=\"text-center text-uppercase text-secondary text-xs font-weight-bolder opacity-7\"> User name </th>
         ...
         while ( $row = mysqli_fetch_array($result2) ){
          ...
          $obj = new Get_nc_name();
          $nc_name = $obj->_check( $row['nc_id'] ) ;

          $obj2 = new Get_u_name();
          $u_name = $obj2->_check( $row['u_id'] ) ;
             $table .= "<tr>
                            <td>
                            <div class=\"d-flex px-2 py-1 \">
                    
                                <div class=\"d-flex flex-column justify-content-center\">
                                <h6 class=\"mb-0 text-sm text-xs lh-lg \"> {$row['uc_id']} </h6>
                                </div>
                            </div>
                            </td>
                            ...
                            <span class=\"text-secondary text-xs font-weight-bold\"> {$nc_name} </span>
                            </td>
                            <td class=\"align-middle text-center\">
                            <span class=\"text-secondary text-xs font-weight-bold\"> {$u_name} </span>
                            </td>
                            <td>
                            <p class=\"text-center text-xs font-weight-bold mb-0\"> {$row['uc_area']} </p>
                            <p class=\" text-center text-xs text-secondary mb-0\"> {$row['uc_area_unit']}  </p>
                            </td>
                            ...
                            <td class=\"align-middle\">           
                             <a class=\"text-xs text-dark me-2 px-2 \" href=\"#\"  onclick=\"myFunction('{$row['uc_id']}')\" ><i class=\"fas fa-pencil-alt text-dark \" aria-hidden=\"true\" ></i></a>                 
                            <a class=\"text-xs text-danger text-gradient \" href=\"../Classes/delete/delete_user_canal.php?uc_id={$row['uc_id']}\" onclick = \"return confirm ('Are you sure you want Delete the Record? ')\"><i class=\"far fa-trash-alt \"></i></a>   
         ...
         $table .= "</table>";
           ...
           // first page
	$first = "<a href=\"table_user_canal.php?p=1\"> <i class=\"fa fa-angle-double-left\"></i>   First  </a>";

	// last page
	$last_page_no = ceil($total_results/$result_per_page);
	$last = "<a href=\"table_user_canal.php?p={$last_page_no}\">Last  <i class=\"fa fa-angle-double-right\"></i></a>";
	...
	if ($page_no >= $last_page_no) {
		...
		$next = "<a href=\"table_user_canal.php?p={$next_page_no}\">Next   <i class=\"fa fa-angle-right\"></i>  </a>";	
	}
	...
	if ($page_no <= 1) {
		...
		$prev = "<a href=\"table_user_canal.php?p={$prev_page_no}\"> <i class=\"fa fa-angle-left\"></i>  Previous</a>";	
	}
