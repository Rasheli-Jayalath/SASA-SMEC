...
?> 
  <main class="main-content mt-1 border-radius-lg">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">User Crop Table </li>
          </ol>
          <h5 class="font-weight-bolder mb-0">User Crop Table</h5>
        </nav>
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          <div class="ms-md-auto pe-md-3 d-flex align-items-center">
          <div class="input-group me-4 ">
              <form action="table_user_crop.php " method="get" id="filter" name="filter" class="">
    ...
    <script>
          ...
          xmlhttp.open("GET","../Scripts/table_ajax/ajax_user_crop.php?q="+str,true);
          xmlhttp.send();
    ...
    $sql1 = "SELECT * FROM wh_002_users_crops WHERE is_deleted = 0  $yr_name ORDER BY cr_id ";
    ...
    $start = ($page_no -1)* $result_per_page;
        $search = '';
        // getting the list of table_user_crop
        if ( isset($_GET['search']) ) {
            $search = mysqli_real_escape_string($connection, $_GET['search']);
            $sql2 = "SELECT * FROM wh_002_users_crops WHERE is_deleted = 0  $yr_name ORDER BY ucp_id ";
        } else {
            $sql2 = "SELECT * FROM wh_002_users_crops WHERE is_deleted = 0  $yr_name ORDER BY ucp_id LIMIT {$start}, {$result_per_page}";
        }
        ...
        <colgroup>
          ...
          $u_name = $obj2->_check( $row['uc_id'] ) ;
             ...
             $table .= "<tr>
                            ...
                            <div class=\"d-flex px-2 py-1 \">
                                ...
                                <h6 class=\"mb-0 text-sm text-xs lh-lg \"> $cr_name </h6>
                                </div>
                            </div>
                            </td>
                            <td>
                            ...
                            <td class=\"align-middle\">           
                             <a class=\"text-xs text-dark me-2 px-2 \" href=\"#\"  onclick=\"myFunction('{$row['ucp_id']}')\" ><i class=\"fas fa-pencil-alt text-dark \" aria-hidden=\"true\" ></i></a>                 
                            <a class=\"text-xs text-danger text-gradient \" href=\"../Classes/delete/delete_user_crop.php?ucp_id={$row['ucp_id']}\" onclick = \"return confirm ('Are you sure you want Delete the Record? ')\"><i class=\"far fa-trash-alt \"></i></a>   
         ...
         $table .= "</table>";
           ...
           // first page
	$first = "<a href=\"table_user_crop.php?p=1\"> <i class=\"fa fa-angle-double-left\"></i>   First  </a>";

	// last page
	$last_page_no = ceil($total_results/$result_per_page);
	$last = "<a href=\"table_user_crop.php?p={$last_page_no}\">Last  <i class=\"fa fa-angle-double-right\"></i></a>";
	...
	if ($page_no >= $last_page_no) {
		...
		$next = "<a href=\"table_user_crop.php?p={$next_page_no}\">Next   <i class=\"fa fa-angle-right\"></i>  </a>";	
	}
	...
	if ($page_no <= 1) {
		...
		$prev = "<a href=\"table_user_crop.php?p={$prev_page_no}\"> <i class=\"fa fa-angle-left\"></i>  Previous</a>";	
	}
